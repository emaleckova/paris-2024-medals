...
# overseas points
points(x = torch_coord[torch_coord$state != "France", ]$long, y = torch_coord[torch_coord$state != "France", ]$lat, col = col_paris[4], cex = 0.85, pch = 20)
# continental france
...
text(torch_coord[torch_coord$city == "Olympia" | torch_coord$state != "France" & torch_coord$city != "Athens" & torch_coord$state != "Martinique", ]$city,
     ...
     col = col_paris[4], cex = 0.5, pos = 2)
text(torch_coord[torch_coord$state == "Guadeloupe", ]$city, x = torch_coord[torch_coord$state == "Guadeloupe", ]$long, y = torch_coord[torch_coord$state == "Guadeloupe", ]$lat, col = col_paris[4], cex = 0.5, pos = 4)
```
...
stage_selection <- as.factor(c(3, 7, 10, 16, 24, 36, 37, 39, 43, 48, 55, 70))
...
points(x = torch_coord$long, y = torch_coord$lat, col = col_paris[4], cex = 0.85, pch = 20)
# label some cities
text(torch_coord[torch_coord$stage_number %in% stage_selection, ]$city, x = torch_coord[torch_coord$stage_number %in% stage_selection, ]$long, y = torch_coord[torch_coord$stage_number %in% stage_selection, ]$lat, col = col_paris[4], cex = 0.5, pos = 2)
```
