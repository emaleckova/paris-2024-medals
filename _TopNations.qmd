...
p_eu <- world |> 
  ...
  st_crop(xmin = -800000, xmax = 3500000, ymin = 3100000, ymax = 7343000) |>
  ggplot() +
  geom_sf(mapping = aes(geometry = geometry), fill = "grey80", colour = "white") +
  # team sizes
  geom_sf_interactive(
    dorl, mapping = aes(geometry = geometry, fill = continent,
                        tooltip = paste0(country_long, ": ", n_participants)),
...
# ranking is only present as a column index (row name)
  ...
  cols_label(
    ...
    iso_a2 = "", 
    country_code = html("Country \n code &#x2195;"), 
    country_long = html("Country &#x2195;"),
  ...
  cols_width(contains("ranking|flag") ~ pct(2),
             contains("country_code") ~ pct(2),
             contains("Medal|Total") ~ pct(5),
             contains("country_code") ~ pct(5),
             everything() ~ pct(70)) |> 
  # add interactivity
  opt_interactive(
...
<!-- gganimate for daily development in time ?-->

<!-- Athletes sent vs with a medal - bar chart sorted by "success" percentage?-->
