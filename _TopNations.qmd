...
# first and last day of medal events are used repeatedly - store them as variables
first_day <- as.Date(min(daily_medals_totals$medal_date))
last_day <- as.Date(max(daily_medals_totals$medal_date))
...
p_cum_medals_cze <- ggplot(daily_medals_cze, aes(x = medal_date, y = cumsum_medals, group = country)) +
  geom_line(linewidth = 1, colour = col_paris[4]) +
  scale_x_date(breaks = seq(first_day, last_day, 1), date_labels = "%m-%d",
               expand = c(0, 0), limits = as.Date(c(first_day, "2024-08-14"))) +
...
athletes_nat |> 
  ...
  ggplot(
    aes(y = Freq,
    axis1 = birth_country, axis2 = nationality)) +
  geom_alluvium()
...
athlete_transfers <- athletes_nat |> 
  ...
  mutate(group = case_when(
    Freq < 2 ~ "rare",
    Freq > 1 & Freq < 6 ~ "minor",
    TRUE ~ paste(birth_country, nationality, sep = " -> ")
  )) |> 
...
ggplot(athlete_transfers, aes(y = Freq, axis1 = birth_country, axis2 = nationality, fill = group)) +
  geom_alluvium() + 
  scale_fill_manual(values = col_transfers) +
  geom_text_repel(
    data = filter(athlete_transfers, group != "rare", group != "minor"),
    aes(x = 2.2, label = group),
    size = 1.5,
    direction = "y",
    xlim = c(2.2, NA),
    hjust = 0,
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(0.6, 3)) +
  #theme_void() +
